...
use Closure;
use XTP_BUILD\Illuminate\Support\Traits\Conditionable;
use XTP_BUILD\Illuminate\Support\Traits\Macroable;
use XTP_BUILD\Illuminate\Support\Traits\Tappable;
use JsonSerializable;
use XTP_BUILD\Symfony\Component\VarDumper\VarDumper;
...
class Stringable implements JsonSerializable
{
    ...
    {
        return new static(XTP_class_basename($this->value));
    }
    ...
    {
        return XTP_collect(explode($delimiter, $this->value, $limit));
    }
    ...
    public function split($pattern, $limit = -1, $flags = 0)
    {
        if (filter_var($pattern, FILTER_VALIDATE_INT) !== false) {
            return XTP_collect(mb_str_split($this->value, $pattern));
        }

        $segments = preg_split($pattern, $this->value, $limit, $flags);

        return ! empty($segments) ? XTP_collect($segments) : XTP_collect();
    }
    ...
    {
        return XTP_collect(sscanf($this->value, $format));
    }
    ...
    {
        return XTP_collect(Str::ucsplit($this->value));
    }
