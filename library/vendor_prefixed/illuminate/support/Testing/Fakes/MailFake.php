...
use Closure;
use XTP_BUILD\Illuminate\Contracts\Mail\Factory;
use XTP_BUILD\Illuminate\Contracts\Mail\Mailable;
use XTP_BUILD\Illuminate\Contracts\Mail\Mailer;
use XTP_BUILD\Illuminate\Contracts\Mail\MailQueue;
use XTP_BUILD\Illuminate\Contracts\Queue\ShouldQueue;
...
class MailFake implements Factory, Mailer, MailQueue
{
    ...
    {
        $mailableNames = XTP_collect($this->mailables)->map(function ($mailable) {
    ...
    {
        $mailableNames = XTP_collect($this->queuedMailables)->map(function ($mailable) {
            return get_class($mailable);
        })->join(', ');
    ...
    {
        return XTP_collect($this->queuedMailables)->filter(function ($mailable) use ($type) {
            return $mailable instanceof $type;
        });
    }
