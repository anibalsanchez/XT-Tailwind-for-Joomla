...
use XTP_BUILD\Illuminate\Contracts\Bus\QueueingDispatcher;
...
class BusFake implements QueueingDispatcher
{
    ...
    public function assertChained(array $expectedChain)
    {
        ...
        PHPUnit::assertTrue(
            XTP_collect($expectedChain)->isNotEmpty(),
    ...
    {
        return XTP_tap(clone $job, function ($job) {
    ...
    {
        $chain = XTP_collect($expectedChain)->map(function ($job) {
    ...
    protected function assertDispatchedWithChainOfClasses($command, $expectedChain, $callback)
    {
        $matching = $this->dispatched($command, $callback)->map->chained->map(function ($chain) {
            return XTP_collect($chain)->map(function ($job) {
    ...
    {
        return ! XTP_collect($chain)->contains(function ($job) {
    ...
    public function dispatchedSync(string $command, $callback = null)
    {
        ...
        return XTP_collect($this->commandsSync[$command])->filter(function ($command) use ($callback) {
            return $callback($command);
        });
    }
    ...
    public function dispatchedAfterResponse(string $command, $callback = null)
    {
        ...
        return XTP_collect($this->commandsAfterResponse[$command])->filter(function ($command) use ($callback) {
            return $callback($command);
        });
    }
    ...
    public function batched(callable $callback)
    {
        ...
        return XTP_collect($this->batches)->filter(function ($batch) use ($callback) {
    ...
    protected function shouldFakeJob($command)
    {
        ...
        return XTP_collect($this->jobsToFake)
