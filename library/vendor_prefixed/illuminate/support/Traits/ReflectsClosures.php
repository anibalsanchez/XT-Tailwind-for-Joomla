...
use Closure;
use XTP_BUILD\Illuminate\Support\Reflector;
...
trait ReflectsClosures
{
    ...
    protected function firstClosureParameterTypes(Closure $closure)
    {
        ...
        $types = XTP_collect($reflection->getParameters())->mapWithKeys(function ($parameter) {
    ...
    protected function closureParameterTypes(Closure $closure)
    {
        ...
        return XTP_collect($reflection->getParameters())->mapWithKeys(function ($parameter) {
            if ($parameter->isVariadic()) {
                return [$parameter->getName() => null];
            }
